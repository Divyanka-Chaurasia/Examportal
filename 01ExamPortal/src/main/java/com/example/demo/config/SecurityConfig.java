...
import com.example.demo.filter.SecurtiyFilter;
@Configuration
@EnableWebSecurity
@CrossOrigin("*")
public class SecurityConfig {
	...
	@Autowired
	private SecurtiyFilter securityFilter;
...
-> web.ignoring().requestMatchers("/v3/api-docs/**");
	...
	public SecurityFilterChain configurePaths(HttpSecurity http) throws Exception{
        http.csrf(csrf -> csrf.disable()).cors(cors -> cors.disable())
                .authorizeRequests(requests -> requests.requestMatchers("/user/**", "/user/login","/user/login/generate-token").permitAll()
